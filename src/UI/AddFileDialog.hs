...
drawUI :: Form DialogState e Name -> [Widget Name]
drawUI f = [C.vCenter $ C.hCenter form]
  where
    form = B.borderWithLabel (str " Create File ") $ padTop (Pad 1) $ hLimit 50 $ renderForm f

app :: App (Form DialogState e Name) e Name
app =
    App
        { appDraw = drawUI
        , appHandleEvent = \ev -> do
            ...
            case ev of
                ...
                _ -> do
                    ...
                    st <- gets formState
                    let nameField = T.unpack $ T.strip $ st ^. name
...
showAddFileDialog :: IO DialogState
showAddFileDialog = do
    let buildVty = do
    ...
    let st = formState f'
    let sanitised = st & name .~ T.strip (st ^. name)
    return sanitised
